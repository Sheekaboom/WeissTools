...
@note split from SamuraiDict on 7/16/2020
...
class WDict(OrderedDict):
    ...
    def load(self,fpath,**kwargs):
        ...
        '''
        my_kwargs = {}
        my_kwargs['object_hook'] = WJSONDecoder
...
class WJSONEncoder(json.JSONEncoder):
    '''
    @brief custom json encoder for specific Weiss Dictionary types  
    '''
...
def WJSONDecoder(o):
    ...
    elif isinstance(o,dict): #if its a dict, make it a WDict
        o = WDict(o)
    return o
...
import unittest
class TestWDict(unittest.TestCase):
    '''@brief unittest class for testing WDict operation'''
    ...
    def test_key_list_update(self):
        ...
        myd = WDict({'test1':'test2',3:{'test':{5:'abc'}}})
        myd[[3,'test',5]] = 'xyz'
    ...
    def test_ed_basic(self):
        ...
        myd = WDict({'test1':'test2','3':WDict({'test':WDict({'5':'abc'})})})
        mydl = WDict() #dict to load
    ...
    def test_ed_function(self):
        ...
        def foo(a,b):
            c = a*b
            return c+a
        myd = WDict({'test1':'test2','3':{'test':{'5':foo}}})
        mydl = WDict() #dict to load
        mydl.loads(myd.dumps())
    ...
    def test_ed_complex_number(self):
        ...
        myd = WDict({'test1':'test2','3':{'test':{'5':complex(1,0)},'t2':complex(3.2,4.1)}})
        mydl = WDict()
        mydl.loads(myd.dumps())
        ...
        myd = WDict({'test1':'test2','3':{'test':{'5':vals_a},'t2':vals_b}})
        mydl = WDict()
        mydl.loads(myd.dumps())
...
if __name__=='__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestWDict)
    ...
    myd3 = WDict({1:{'test1':{'myfun':foo}}})
    myd4 = WDict()
    myd4.loads(myd3.dumps()) #try and load a function
    myd4['1']['test1']['myfun']
    json.loads(myd3.dumps(),object_hook=WJSONDecoder)
